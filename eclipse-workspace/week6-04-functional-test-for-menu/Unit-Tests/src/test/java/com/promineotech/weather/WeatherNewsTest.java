...
class WeatherNewsTest {
  ...
  private WeatherNews createWeatherNewsMock(Weather weather) {
    ...
    lenient().doNothing().when(news).alertHighTemp(weather); // doNothing bc method returns void
    lenient().doNothing().when(news).alertLowTemp(weather); // doNothing bc method returns void
  ...
  private Weather createHighLowWeatherEvent() {
    ...
    Float high = WeatherNews.getHighAlertTrigger() + 0.1F; // add 0.1 degree to high alert trigger value
    Float low = WeatherNews.getLowAlertTrigger() - 0.1F; // subtract 0.1 degree from low alert trigger value
    String report = region + " high=" + high + ", low=" + low;
